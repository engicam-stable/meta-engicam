	...
	do {
		...
		switch (opt) {
			...
			case 'i':
				...
				break;
			case 'd':
				g_v4l_device = strdup(optarg);
				break;
			case 'V':
				printf("vadc_pxp: version %s\n", VERSION);
				exit(0);
...
int vadc_test(void *arg) {
	...
	if (stop_capturing(fd_v4l) < 0)
	{
		...
		dbg(DBG_INFO, "vdac_pxp finished!\n");
	}
