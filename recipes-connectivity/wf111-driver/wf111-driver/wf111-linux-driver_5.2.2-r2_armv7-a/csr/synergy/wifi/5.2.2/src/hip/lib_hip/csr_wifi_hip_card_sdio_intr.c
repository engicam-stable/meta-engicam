...
CsrResult unifi_bh(card_t *card, CsrUint32 *remaining)
{
    ...
    do
    {
        ...
        /*
         * Need to check if the HIP should run again. The check is needed
         * because of a race condition regarding the variable
         * card->dynamic_slot_data.from_host_used_slots which is used in the
         * define FROM_HOST_USED_SLOTS. The variable is updated by the HIP but
         * the define FROM_HOST_USED_SLOTS can be used in the function
         * send_signal in the file csr_wifi_hip_send.c after the HIP has started
         * handling the interrupt but before the variable is updated, and we
         * could be waiting for an interrupt that has already happened.
         */
        run_bh = 0;
        ...
        if (from_host_used_slots)
        {
            ...
            {
                rb = 1;
                RUN_BH(card->fh_traffic_queue, card, i, rb);
                run_bh |= rb;
            }
        }
    } while (run_bh);
...
static CsrResult handle_host_protocol(card_t *card, CsrBool *processed_something)
{
    ...
    *processed_something = FALSE;
    ...
    napi_schedule_new_packets(card->ospriv);
#endif
#endif
...
#error "CSR_WIFI_ALIGNMENT_WORKAROUND is not supported when CSR_WIFI_SDIO_USE_QUEUE is in use."
#endif
#ifndef CSR_WIFI_BULK_TRANSFER_FIRST
#error "CSR_WIFI_BULK_TRANSFER_FIRST is needed to be able to use CSR_WIFI_SDIO_USE_QUEUE. (CSR_WIFI_BULK_TRANSFER_FIRST can however be used without CSR_WIFI_SDIO_USE_QUEUE)"
#endif
static void cardClearFromHostDataSlotDeferred(card_t *card, const CsrUint8 *cmdptr)
{
    CsrInt16 slot;
 ...
 *
 *      CSR_WIFI_BULK_TRANSFER_FIRST will cause the the signal queue to be passed
 *      two times, and provides the same dynamics as the CSR_WIFI_RX_PATH_SPLIT
 *      but adds the possibility to chain SDIO operations:
 *      1) In the first pass (signals are ignored):
 *         - all bulk transfers are extracted, and either executed or added to the
 *           sdio-cmd chain including any flush operations.
 *         - all bulk transfers are executed.
 *      2) In the second run the signal queue elements are handled.
 *
 *      CSR_WIFI_BULK_TRANSFER_FIRST together with CSR_WIFI_SDIO_USE_QUEUE
 *      should be used on systems with large context switch overhead, such as
 *      when using the Linux MMC driver, which is a high-latency driver not
 *      suited to be used for small transfers.
 *
 * ---------------------------------------------------------------------------
 */
static CsrResult process_to_host_signals(card_t *card, CsrInt32 *processed)
{
    ...
    CsrResult r = CSR_RESULT_SUCCESS;
#ifdef CSR_WIFI_BULK_TRANSFER_FIRST
    CsrBool doingBulk = FALSE; /* This will toggle in the top of the while loop, causing the loop to run twice */
    CsrUint16 initialCount = card->th_buffer.count;
#endif
    ...
    {
#ifdef CSR_WIFI_BULK_TRANSFER_FIRST
        doingBulk = doingBulk?FALSE : TRUE;   /* Toggle the doing bulk indicator.
                                                 First run: TRUE, second run: FALSE*/
#endif
        *processed = 0;
        ...
        {
#ifdef CSR_WIFI_SDIO_USE_QUEUE
            CsrWifiSdioQueueFlush();   /* Reset the Building Queue state. */
#endif
            return CSR_RESULT_SUCCESS;
        }
...
#ifndef CSR_WIFI_BULK_TRANSFER_FIRST /* All the bulk transfers have already been flushed */
                                     /*
...
#ifdef CSR_WIFI_SDIO_USE_QUEUE
                    else /* We are now done with the bulk transfers. */
                    { /* Clear the FromHost data pointers if we use mmc-request queue. */
                        if (cmd == SDIO_CMD_FROM_HOST_AND_CLEAR)
                        {
                            cardClearFromHostDataSlotDeferred(card, bufptr);
                        }
                    }
#endif
                    break;
...
#endif
#ifdef CSR_WIFI_BULK_TRANSFER_FIRST
            card->th_buffer.count = initialCount; /* Rewind the signal count for the second pass */
#endif
        }
    }
#ifdef CSR_WIFI_BULK_TRANSFER_FIRST
    while (doingBulk == TRUE); /* Pass the signal queue twice */
#else
    while (0);                 /* only once */
#endif

    card->to_host_signals_r = (card->to_host_signals_r + (card->th_buffer.count - pending)) % 128;
...
#ifndef CSR_WIFI_SDIO_USE_QUEUE
    if (cmd == SDIO_CMD_FROM_HOST_AND_CLEAR)
    {
        if (slot >= card->config_data.num_fromhost_data_slots)
        {
...
#else
    /* We do not clear the slot until after the chained commands are executed. */
#endif
